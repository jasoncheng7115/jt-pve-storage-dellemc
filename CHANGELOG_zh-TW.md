...
### 文件
- **升級對舊版建立的磁碟區有什麼影響** —— 答案是沒有影響 —— 現在明白寫下來，而不是隱含
  在其他敘述裡。`docs/NAMING_CONVENTIONS_zh-TW.md` 新增一節「**升級**」，列出舊版可能
  建立過的每一種物件（磁碟、EFI、TPM 狀態、cloud-init、設定備份、快照、範本標記），並
  逐一確認它們仍然被認得。文件站也新增了自己的「升級」章節，並放進側邊欄，中英兩語皆有。

  那一節同時記下了「什麼都不會做」的部分，因為「我們不會動它」本身就是一項值得明說的
  聲明：孤兒回收器只處理節點自己的裝置，從不刪除儲存伺服器上的磁碟區；暫時複製回收器
  依據的是外掛自己寫下的狀態檔，而舊磁碟區不在其中；每 VM 一個 volume group 預設關閉，
  也不會把既有磁碟區追溯加入群組。

  以及升級後**確實會**改變的兩件事，而兩者都是修正：第一次使用時可能會建立 multipath
  map，所以在日誌中看到對既有磁碟區執行 `multipath -a` 是預期的；以及加入既有 storage
  時會警告該前綴下已經有磁碟區，那是跨叢集檢查在你重新加入自己的 storage 時正常運作。

- 「前綴隔離」那一節原本寫著每個物件都以 `pve-<storeid>-` 命名。這在 0.8.29 前置字串
  變成可設定之後就不再成立了。現在改為 `<前置字串>-<storeid>-`，並註明除非 storage 有
  設定，否則前置字串就是 `pve`，同時說明為什麼它是一個由操作者設定的值、而不是叢集名稱。

## [0.8.30~beta1] - 2026-08-27

### 已修正
- **復原工具不知道 `dell-name-prefix`**，那是前一個版本才加入的。對一個有設定它的
  storage，`pve-dell-config-get` 會**完全找不到任何備份**：它要找的每一個名稱都以那個
  字首開頭，而它仍然用著 `pve`。這不是錯誤，是一份空清單 —— 而且發生在有人正因為 PVE
  起不來、要復原 VM 設定的那一刻。

  它現在會從 `storage.cfg` 讀這個鍵，並且接受 `--name-prefix`，供那種連 `storage.cfg`
  都讀不到的復原情境使用 —— 而那正是這個工具存在的理由。

  **這是第三次了。** 這個工具刻意重新實作了外掛所做的事，因為它必須在儲存層失效時仍然
  能用；而那份獨立性，正是它會脫隊的原因。密碼移出 `storage.cfg` 時它被留下過一次，
  `dell-host-mode shared` 時又一次。任何改變「外掛命名什麼」或「外掛從哪裡讀」的東西，
  都屬於它的檢查清單，而這次兩者皆是。

  這是從一個「新安裝會不會看到舊安裝建立的磁碟區」的提問查出來的 —— 那是另一個問題，
  但一路查到了這裡。

### 測試
- 這個修正的第一版測試，斷言的是 `--name-prefix` 有出現在 `--help` 裡，而它在「選項有
  寫進說明、卻沒有接上 `GetOptions`」的情況下**照樣通過** —— 在那個腳本裡，說明文字與
  `GetOptions` 是兩個不同的地方。現在它會真的把選項傳給工具。是靠把修正種回去才看出差別
  的：一個你沒讀懂為什麼會通過的測試，不算測試 —— 而這句話本來就已經寫在這個檔案上。
...
### 已修正
- **線上遷移的目標節點會讓客體跑在單一路徑上。** 一台第一次看到某個磁碟區的節點，會先
  認領 WWID，然後**立刻**檢查 map。但 `multipathd add path` 是非同步的，它在 map 存在
  之前就返回了，所以那個檢查幾乎必定落空，外掛就退回它一開始拿到的那條裸 `/dev/sdX`。
  而遷移目標正是「節點第一次看到這個磁碟區」的情境，所以那在那裡是常態，不是邊角案例。

  現在它會等待 map 出現，上限 5 秒，而且只在完全沒有 map 時才等 —— 一旦有了 map，前面
  的快速路徑就會先返回，所以已經在跑這個磁碟區的節點永遠不會等。

  由 **Alexander Gott（[@alexandergott-afk](https://github.com/alexandergott-afk)）**
  在 [issue #7](https://github.com/jasoncheng7115/jt-pve-storage-dellemc/issues/7)
  回報，而他的判讀是對的：`multipath -a` 之後，map 需要一點時間才會可用。

  這是 0.8.26 那個修正引入的，只隔一個版本；而它上面的註解寫著「給 multipathd 一點
  時間」，程式碼卻一點都沒給。

## [0.8.27~beta1] - 2026-08-26

### 新增
- **加入儲存時，會去問儲存伺服器「這個 storage 的磁碟區名稱字首是不是已經有人在用」**，
  有的話就發出警告。

  兩個 Proxmox 叢集接到同一台儲存伺服器、而且都有一個叫 `ps001` 的 storage 時，它們會
  共用同一個磁碟區命名空間：所有名稱都是從 storage id 組出來的，於是兩邊會互相列出對方
  的磁碟，而在其中一邊刪除也可能刪到另一邊。既有的檢查在**同一個叢集內**正是為此而拒絕，
  但它讀的是本機的 `storage.cfg`，看不到另一個叢集。

  由 **Alexander Gott（[@alexandergott-afk](https://github.com/alexandergott-afk)）**
  在 [issue #4](https://github.com/jasoncheng7115/jt-pve-storage-dellemc/issues/4)
  提出。

  這是**警告，絕不是拒絕**。重新加入一個已經有磁碟區的 storage 是正當的（重灌之後、或
  `pvesm remove` 之後），而只有操作者分得出那與撞名的差別。連不上的儲存伺服器則什麼都不
  說，也不會拖慢加入：指向一個不可路由的位址實測 0.64 秒。

### 刻意不做的
- **磁碟區名稱字首不會加入叢集名稱**，而那是這個回報最初要求的。三個理由，第三個是決定性
  的：

  - storage id 之所以是命名空間，是因為**一個叢集可能在同一台儲存伺服器上有好幾個
    storage**，所以它無論如何都必要。叢集名稱只會是附加，不是取代。
  - 改變命名格式會讓外掛**認不得它自己已經建立的每一個磁碟區**。一次升級就會讓儲存
    伺服器上所有磁碟變成孤兒。
  - 為了塞得下而截斷過長的叢集名稱 —— 而空間緊的家族正需要如此，PowerVault 只剩 4 個
    字元、PowerFlex 4 個 —— 會把 `production-a` 與 `production-b` 映射到同一個字首。
    那會**在看起來解決了的同時把撞名原封不動地變回來**，比沒有更糟；而要偵測它，需要的
    正是上面新增的那個檢查。
...
### 已修正
- **透過 host group 對應的磁碟區刪不掉。** 儲存伺服器以「仍在掛載中」為由拒絕，而它
  說得沒錯：`_array_mapped_hosts` 只讀 `host_id`，而**群組層級的
  `host_volume_mapping` 資料列只帶 `host_group_id`、沒有 `host_id`**。於是磁碟區看起來
  沒有任何對應，呼叫端什麼也沒解除，刪除就被拒絕。

  這件事本專案早就寫下來過 —— 寫在 LUN 配發那一段，因為群組對應會在群組的每一個成員
  上佔用該編號。只是從來沒有套用到解除對應這條路徑。現在群組對應會以它其中一個成員
  host 的名稱回報，那正是解除對應路徑接受的形式，而透過任一成員解除時指名的就是群組。

  由 **Alexander Gott（[@alexandergott-afk](https://github.com/alexandergott-afk)）**
  在 [issue #11](https://github.com/jasoncheng7115/jt-pve-storage-dellemc/issues/11)
  回報。

- **刪除被拒時的訊息，斷言了一個它並未查證的原因。** 它拿儲存伺服器的錯誤去比對
  `/clone|dependent|child|in use/`，一旦命中就宣稱有人從這個磁碟區做過精簡複製。而它
  比對到的字串，是**外掛自己附加的那句 422 提示** —— 那句話寫著「……或仍有快照或精簡
  複製依賴著它」，所以每一個 422 都會命中，不論儲存伺服器實際說了什麼。一個因為還掛在
  host group 上而被拒絕的磁碟區，害操作者去找根本不存在的精簡複製。

  現在會直接把儲存伺服器自己的說法交給操作者，而不是替它做總結；可能的原因則以「可能」
  的形式列出，而不是當成已經查明的結論。

  這是**教訓 18 的第二次**：絕不要去比對一則有本外掛參與撰寫的訊息。第一次它讓範本永遠
  刪不掉，原因相同，比對到的也是同一句提示。
...
### 新增
- **`dell-host-mode host-group`**（僅 PowerStore）。它保留各節點的 host 物件，讓儲存
  伺服器仍然能回報每台節點的連線狀態，同時把它們放進一個名為 `pve-<叢集>-cluster`
  的 host group，讓一次對應就能觸及每一台節點。由
  **Alexander Gott（[@alexandergott-afk](https://github.com/alexandergott-afk)）**
  ...
  提出，而設計中最難的那一部分也是他定案的。

  整個設計由 Dell 自己用戶端程式碼裡的一個事實決定：**一個 host 最多只能屬於一個
  host group。** host 物件帶的是 `host_group_id`，單數；而磁碟區帶的是
  `volume_groups`，是清單。而且位於群組中的 host 是**透過**該群組被對應的。所以加入
  我們的群組，就等於離開它原本所在的群組，而那會拿走那個群組對應給它的每一個磁碟區
  —— 何況這個搬移連原子性都沒有，因為 `add_host_ids` 與 `remove_host_ids` 在同一次
  請求中互斥。

  因此：不在任何群組的 host 會被加入，已經在我們群組裡的不動，而**在別人群組裡的
  host 永遠不會被搬動**。它只會被回報一次，磁碟區則繼續透過那個群組對應，而那是可行
  的。那台 host 只能服務 Proxmox 或另一個工作負載，不能兩者兼具，而要選哪一個不是
  外掛的決定。移除永遠不會自動發生；而且只有外掛自己建立、並以描述中的標記為憑的
  群組，才可能被刪除。

- **加入儲存時會自動偵測叢集名稱**，從 `/etc/pve/corosync.conf` 讀取，並寫進該儲存的
  設定裡。[issue #4](https://github.com/jasoncheng7115/jt-pve-storage-dellemc/issues/4)。

  既有的儲存則刻意不動。叢集名稱是 host 物件名稱的一部分，而一個 initiator 在這些
  儲存伺服器上只能屬於一個 host 物件；如果改成在每次啟用時重新推導，那麼每一個沿用
  `pve` 預設值的儲存都會突然找不到自己的 host、去建立一個新的，然後儲存伺服器會拒絕
  那些 initiator，或是把它們從「每台節點的磁碟區都對應過去的那個物件」上搬走。在儲存
  剛建立、儲存伺服器上還沒有任何東西以它命名的那一刻決定，是唯一不需付出代價的時機。
  回報者也確認不需要遷移。

### 已修正
- **名稱被拒絕時的訊息，現在會說出是什麼佔住了它。** 當儲存伺服器拒絕一個它自己的
  列表顯示為空的名稱時，外掛會去查 `/recycle_bin`，並指出那個物件、它的 id 與刪除
  時間。

  這些端點是回報者提供的，而這也更正了 0.8.20 的推理。那一版寫著「Dell 自己的 SDK
  沒有回收筒端點，所以沒有可靠的東西可問」。前半句是真的 —— `python-powerstore`
  確實沒有包裝它 —— 但結論是錯的：`/recycle_bin` 自 PowerStore 3.5.0.0 起就存在，而
  一個 SDK 沒有包裝某個端點，完全不能說明儲存伺服器有沒有它。**儲存伺服器自己的 API
  參考，才是綁著實際運行韌體的那個來源** —— 這條教訓本專案早已為 Unity 寫下來過，
  只是這次沒有拿來用。

  這個查詢是**唯讀**的。為了空出一個名稱而永久刪除別人被回收的磁碟區，不是這個外掛
  該做的決定；回收筒的存在，正是為了讓刪除是一件刻意的事。

### 已移除
- 一個沒有人使用的 `encode_host_group_name`，它替 host group 取的名字與 shared 模式的
  **host** 物件完全相同。除了測試之外沒有任何地方呼叫它，而它源自 0.8.22 已更正的那個
  誤解 —— 以為 shared 模式會建立 host group。
...
### 已修正
- **一個錯誤的解釋，寫在程式碼註解裡，也寫在 0.8.19 的變更紀錄裡。** 兩處都說核心那句
  *LUN assignments on this target have changed*，是殘留的 sd 路徑在 LUN ID 被重用時
  造成的。

  並不是。那句話是任何儲存伺服器在自己的 LUN 清單變動時都會送出的 unit attention，
  所以本外掛**每一次對應與每一次解除對應**都會觸發一次，其他廠商的儲存伺服器也一樣。
  用來做本機驗證的那台節點只有 NetApp iSCSI、完全沒有設定任何 Dell storage，而它在
  六十天內記錄了這句話 **396 次**。

  [issue #7](https://github.com/jasoncheng7115/jt-pve-storage-dellemc/issues/7)
  的回報者在升級 0.8.19 之後回來說訊息還在出現，這才讓錯誤的解釋被抓到。在此致謝。

  0.8.19 加入的那項清理仍然成立，而且值得做，只是理由與當初給的不同：
  `_release_volume` 留下的 sd 路徑仍然描述著「曾經在那個 LUN ID 上」的磁碟區，而
  `next_free_lun` 取的是最低的可用號碼，所以會有另一個磁碟區來到那個位置，而 SCSI
  層不會自行更正一個「身分已經對不上背後實體」的裝置節點。那才是真正的風險，而那句
  核心訊息從來不是偵測它的方法。

  上面 0.8.19 那一則採取加註更正、而不是改寫，理由與 0.7.88 那一則相同：變更紀錄是一
  項聲明，而被悄悄改掉的聲明，會讓人看不出它曾經被說過。

## [0.8.20~beta1] - 2026-08-24

### 已修正
- **建立虛擬機時可能出現十次一模一樣的重試，然後歸咎於根本不存在的其他節點。**
  在 PowerStore Manager 中刪掉的磁碟區會留在回收筒裡：任何列表都看不到它，而儲存
  伺服器仍然拒絕它的名字。

  `alloc_image` 會挑一個磁碟 ID，建立被拒絕時再挑一次 —— 而挑的方式是去問儲存
  伺服器哪些 ID 是空的。那正是回收筒讓它出錯的那個視角。於是列表說 `disk0` 是空的、
  建立說它已被佔用，下一輪又問同樣的問題、得到同樣的答案，如此十次，日誌每一行都寫著
  *retrying as* 它剛剛失敗的那個名字。最後的訊息還在一台單節點的機器上歸咎於
  *allocations from other nodes*，並建議重試一件永遠不會成功的事。

  現在重試會記住哪些 ID 被拒絕過並跳過它們，因此不論是什麼東西佔住那個名字，它都會
  收斂。這刻意不是去查詢那個被回收的磁碟區：Dell 自己的 SDK 沒有任何回收筒端點，
  所以沒有可靠的東西可問；而且下一個以這種方式隱形佔住名字的東西，也不會是回收筒。

  兩種失敗現在也被分開了。輸掉與其他節點的競賽是值得重試的，訊息就這麼說；而儲存
  伺服器拒絕一個它自己的列表顯示為空的名字，則不值得重試，訊息會指出回收筒是該檢查
  的地方。

  由 **Alexander Gott（[@alexandergott-afk](https://github.com/alexandergott-afk)）**
  在 [issue #9](https://github.com/jasoncheng7115/jt-pve-storage-dellemc/issues/9)
  回報。在此致謝。

### 已變更
- PowerFlex 自己的 `_find_free_diskid` 也接受相同的 `exclude` 參數。它不繼承
  `BlockBase`，所以兩邊的簽章是靠人工保持一致，而不是靠繼承。
- `docs/RELEASE_TESTING.md` 新增了這些回報所產生的實機測試項：預設
  `find_multipaths strict` 的節點、刪除快照後殘留的 sd 路徑、快照期間的客體凍結、
  回收筒，以及 UEFI 磁碟。另外還有每 VM 一個 volume group 的檢查項，包括把外掛的
  某個磁碟區放進你自己的群組，確認它仍然刪得掉。中英兩份都更新。
...
- **`_release_volume` 完全沒有做本機裝置清理。** 那是設定備份磁碟區、以及讀取快照用的
  暫時複製所走的刪除路徑，而它留下來的 sd 路徑，仍然描述著「曾經在那個 LUN ID 上」的
  磁碟區；而 `next_free_lun` 取的是最低的可用號碼，所以會有另一個磁碟區來到那個位置。
  `cleanup_lun_devices` 早就存在，只是這條路徑從來沒有呼叫過它。回報於
  [issue #7](https://github.com/jasoncheng7115/jt-pve-storage-dellemc/issues/7)。

  > **已於 0.8.21 更正。** 這一則原本寫著那些 sd 路徑「正是核心那句
  > *LUN assignments on this target have changed* 的成因」。並不是。那句話是任何
  > 儲存伺服器在 LUN 清單變動時都會送出的 unit attention。這項清理仍然值得做，
  > 但那句訊息從來不是它的指標。
