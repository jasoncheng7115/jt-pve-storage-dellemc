...
| `host_group_id` (on a host), `hosts`, `host_ids`, `add_host_ids`, `remove_host_ids` | host groups, for `dell-host-mode host-group`. **`host_group_id` is singular where a volume's `volume_groups` is a list**, which is what says a host belongs to at most one group; `add_host_ids` and `remove_host_ids` are mutually exclusive in one PATCH in Dell's own client, so a move between groups is not atomic. Names read from `python-powerstore`'s `modify_host_group` |
